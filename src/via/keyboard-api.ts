...
// 从 HID 设备对象提取设备信息(替代原 Redux errorsSlice 中的 extractDeviceInfo)
const extractDeviceInfo = (device: any): any => ({
  productId: device?.productId ?? -1,
  vendorId: device?.vendorId ?? -1,
  productName: device?.productName ?? 'Unknown',
});

const getMessageFromError = (e: any): string =>
...
export class KeyboardAPI {
    ...
    while (this.commandQueueWrapper.commandQueue.length !== 0) {
      ...
      if (typeof args === 'function') {
        ...
        try {
          ...
          console.error('KeyboardAPI command error:', {
  ...
  async _hidCommand(
    ...
    if (!eqArr(commandBytes.slice(1), bufferCommandBytes)) {
      ...
      const deviceInfo = extractDeviceInfo(this.getHID());
      console.error('KeyboardAPI incorrect response:', {
