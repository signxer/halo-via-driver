import type {DeviceInfo, WebVIADevice} from './types';
...
const ExtendedHID = {
  ...
  // 仅列出已授权的设备(不弹选择器),供页面加载时自动连接
  getAuthorizedDevices: async () => {
    const hidDevices = await ExtendedHID.getFilteredDevices();
    return hidDevices.map(tagDevice);
  },
  ...
  HID: class HID {
    ...
    read(fn: (err?: Error, data?: ArrayBuffer) => void) {
      ...
      if (globalBuffer[this.path].length > 0) {
        ...
        eventWaitBuffer[this.path].push((data) => fn(undefined, data as any));
      }
    }
